...
function normalizarImovelPublico(imovel) {
  const preco = Number(imovel.preco || 0);
  const precoLabel = imovel.precoLabel || (preco >= 1000000
    ? `R$ ${(preco / 1000000).toFixed(2).replace(".", ",")} mi`
    : `R$ ${Math.round(preco / 1000)} mil`);
  return {
    id: imovel.id || imovel.slug || imovel.codigo,
    codigo: imovel.codigo || imovel.slug || (imovel.id ? `PV-${imovel.id}` : ""),
    bairro: imovel.bairro || "Vitória da Conquista",
    tipo: imovel.tipo || "Imóvel",
    titulo: imovel.titulo || "Imóvel selecionado pela busca",
    descricao: imovel.descricao || "Opção encontrada conforme os filtros informados.",
    preco,
    precoLabel,
    quartos: imovel.quartos ?? 0,
    suites: imovel.suites ?? 0,
    vagas: imovel.vagas ?? 0,
    area: imovel.area || imovel.area_util || 0,
    img: imovel.img || imovel.imagem || "https://images.unsplash.com/photo-1560518883-ce09059eeffa?w=1200&q=80",
    tags: imovel.tags || imovel.caracteristicas || [],
    status: imovel.status || (imovel.destaque ? "Destaque" : "Disponível"),
    iaMatch: imovel.iaMatch || imovel.score || 88,
    matchCalculado: imovel.matchCalculado,
    temas: imovel.temas || [],
  };
}
...
function PropertyCard({ imovel, variant }) {
  const ref = useTilt();
  const card = normalizarImovelPublico(imovel);
  const linkDetalhe = card.codigo ? `#/imovel/${String(card.codigo).toLowerCase()}` : "#contato";
  const matchValor = typeof card.matchCalculado === "number" ? card.matchCalculado : card.iaMatch;
  const matchLabel = card.matchCalculado != null
  ...
  const compartilhar = (e) => {
    ...
    const linha2 = `${card.titulo} — ${card.precoLabel}`;
    ...
    <article className={`pcard pcard-${variant}`} ref={ref}>
      <div className="pcard-inner">
        <div className="pcard-photo-wrap">
          <img src={card.img} alt={card.titulo} className="pcard-photo" loading="lazy"/>
          <div className="pcard-photo-grade"/>
          <div className="pcard-tags">
            <span className="pcard-status">{card.status}</span>
            <span className="pcard-match" title={matchLabel}>
              ...
              {matchValor}% match{card.matchCalculado != null ? "" : " IA"}
            </span>
          </div>
          <div className="pcard-bairro">{card.bairro}</div>
          {card.codigo && <div className="pcard-codigo">{card.codigo}</div>}
        </div>
        <div className="pcard-body">
          <div className="pcard-head">
            <div className="pcard-tipo">{card.tipo}</div>
            <div className="pcard-preco">{card.precoLabel}</div>
          </div>
          <h3 className="pcard-titulo"><a href={linkDetalhe} className="pcard-titulo-link">{card.titulo}</a></h3>
          <p className="pcard-desc">{card.descricao}</p>
          <div className="pcard-stats">
            <span><strong>{card.quartos}</strong> quartos</span>
            <span><strong>{card.suites}</strong> suítes</span>
            <span><strong>{card.vagas}</strong> vagas</span>
            <span><strong>{card.area}</strong> m²</span>
          </div>
          <div className="pcard-actions">
            ...
            {window.BotaoComparar
              ? <window.BotaoComparar codigo={card.codigo}/>
              : null}
            {window.BotaoFavoritar
              ? <window.BotaoFavoritar codigo={card.codigo} variant={variant}/>
...
function PropertyGrid({ filter, resultadoBusca, variant = "cerulean", title = "Imóveis em destaque", subtitle, emptyMessage }) {
  const filtroAtivo = !!(filter && (filter.bairro || filter.tipo || filter.faixa || filter.tema));
  const imoveis = React.useMemo(() => {
    if (resultadoBusca?.imoveis) {
      return resultadoBusca.imoveis.map(normalizarImovelPublico);
    }
